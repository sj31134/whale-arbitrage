...
4개 거래소 (업비트, 바이낸스, 비트겟, 바이비트) 공통 날짜만 사용
"""
...
def prepare_aligned_data(start_date="2024-01-01", end_date=None):
    ...
    query = f"""
        ...
        INNER JOIN bybit_spot_daily bb ON u.date = bb.date AND bb.symbol = 'BTCUSDT'
    ...
    if common_dates_df.empty:
        print("⚠️ 공통 날짜가 없습니다. 각 거래소 데이터 기간을 확인하세요.")
        conn.close()
        return None
    ...
    aligned_query = f"""
        SELECT 
            ...
            bb.open as bybit_open,
            bb.high as bybit_high,
            bb.low as bybit_low,
            e.krw_usd
        FROM upbit_daily u
        INNER JOIN binance_spot_daily b ON u.date = b.date AND b.symbol = 'BTCUSDT'
    ...
    print(f"   바이비트 NULL: {df['bybit_price'].isnull().sum()}건")
...
def save_aligned_data(df, output_path=None):
    ...
    if output_path is None:
        output_path = ROOT / "data" / "aligned_4exchanges_data.csv"
...
if __name__ == "__main__":
    # 4개 거래소 공통 데이터 기간으로 조정
    df = prepare_aligned_data(start_date="2025-05-12")
    ...
    if df is not None:
        ...
        print(f"   바이비트 평균 가격: {df['bybit_price'].mean():,.2f} USDT")
    else:
        print("\n❌ 데이터 정렬 실패")
