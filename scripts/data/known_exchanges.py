"""
Known exchange addresses for fallback labeling.
"""
...
def get_known_label(address, chain_type):
    """
    Returns the known label for an address if it exists in the static list.
    """
    address = address.lower()
    for exchange in KNOWN_EXCHANGES:
        if exchange['address'].lower() == address and exchange['chain_type'] == chain_type:
            return exchange['name_tag']
    return None
